...
@synthesize currentMonth,delegate,labelCurrentMonth, animationView_A,animationView_B,markedDates;
@synthesize markedColors,calendarHeight,selectedDate, selday;
...
-(void)selectDate:(int)date {
   ...
   /* if (selectedDateYear < currentMonthYear) {
    ...
    if(selectedDateMonth == currentMonthMonth)
    {
        
        
        NSDate *todayDate = [NSDate date];
        
        if(currentMonthMonth == [todayDate month])
        {
           if([selectedDate day] >= [todayDate day] )
            if ([delegate respondsToSelector:@selector(calendarView:dateSelected:)])
            {
                ...
                NSString * dateFormatted = [dateFormatter stringFromDate:selectedDate];
                
                 NSDate * dateNotFormatted = [dateFormatter dateFromString:dateFormatted];
               
                 NSDateFormatter * dateFormatter1 = [[NSDateFormatter alloc]init];
                 [dateFormatter1 setDateFormat:@"MM/dd/yyyy"];
                
                NSString * finalDate = [dateFormatter1 stringFromDate:dateNotFormatted];
                
                [delegate calendarView:self dateSelected:finalDate];
                
                NSLog(@"%d", [selectedDate day]);
            }
        }
       else
       {
...
{       
    ...
    if (CGRectContainsPoint(rectArrowLeft, touchPoint)) {
        if(month_count <= 4 && month_count > 0)
        {
            [self showPreviousMonth]; month_count--;
        ...
        if(month_count <= 3)
        {
           [self showNextMonth]; month_count++;
...
- (void)drawRect:(CGRect)rect
{
    ...
    labelCurrentMonth.layer.cornerRadius = 24;
    labelCurrentMonth.clipsToBounds = YES;
    ...
    CGContextSetFillColorWithColor(context, 
                                   [UIColor colorWithHexString:@"0xe7e7e7"].CGColor);
    for (int i =0; i<[weekdays count]; i++) {
        ...
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth+2), 65, kVRGCalendarViewDayWidth+2, 20) withFont:font lineBreakMode:UILineBreakModeClip alignment:NSTextAlignmentCenter];
    }
  ...
  /*  float gridHeight = numRows*(kVRGCalendarViewDayHeight+2)+1;
    ...
    NSDate *todayofmonth = [NSDate date];
    
   // NSLog(@" current Month = %d -------- current Year = %d ",[todayofmonth month],[todayofmonth year]);
    
  //  NSLog(@" previous Month = %d -------- previous Year = %d ",[previousMonth month],[previousMonth year]);
    
    
    if(([previousMonth month] >= [todayofmonth month] && [previousMonth year] >= [todayofmonth year]) ||([previousMonth month] < [todayofmonth month] && [previousMonth year] > [todayofmonth year]))
    {
            ...
            CGSize finalSize;
            UIImage* nextmonthBtn = [UIImage imageNamed:@"premonthBtn.png"];
            
            
            UIGraphicsBeginImageContextWithOptions(CGSizeMake(50, 40), NO, 0.0);
            UIGraphicsPushContext(context);
            [nextmonthBtn drawInRect:CGRectMake(0, 3, 50, 40)];
            UIGraphicsPopContext();
            UIImage *outputImage = UIGraphicsGetImageFromCurrentImageContext();
            UIGraphicsEndImageContext();
            
            finalSize = [outputImage size];
            UIGraphicsBeginImageContext(finalSize);
            [outputImage drawInRect:CGRectMake(0,0,50,40)];
            UIGraphicsEndImageContext();
    ...
    if(month_count <= 3)
    {
        ...
        CGSize finalSize;
        UIImage* nextmonthBtn = [UIImage imageNamed:@"nextmonthBtn.png"];
        
        
        UIGraphicsBeginImageContextWithOptions(CGSizeMake(50, 40), NO, 0.0);
        UIGraphicsPushContext(context);
        [nextmonthBtn drawInRect:CGRectMake(kVRGCalendarViewWidth-50, 3, 50, 40)];
        UIGraphicsPopContext();
        UIImage *outputImage = UIGraphicsGetImageFromCurrentImageContext();
        UIGraphicsEndImageContext();
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (i<firstWeekDay) { //previous month
            ...
            NSString *hex = (isSelectedDatePreviousMonth) ? @"0x383838" : @"5686a7";
            ...
            if(targetDate < [todayDate day])
            {
                if(([self.currentMonth month] > [todayofmonth month] && [self.currentMonth year] >= [todayofmonth year]) ||([self.currentMonth month] < [todayofmonth month] && [self.currentMonth year] > [todayofmonth year]) )
                 hex =  @"0xe7e7e7";
                else
                  hex =  @"5686a7";
            }
        ...
        [date drawInRect:CGRectMake(targetX, targetY+30, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:@"HelveticaNeue-Light" size:27] lineBreakMode:UILineBreakModeClip alignment:NSTextAlignmentCenter];
    }
...
-(id)init {
    ...
    self = [super initWithFrame:CGRectMake(260, 160, kVRGCalendarViewWidth, 0)];
    if (self) {
        ...
        isAnimating=NO;        
        self.labelCurrentMonth = [[UILabel alloc] initWithFrame:CGRectMake(34, 40, kVRGCalendarViewWidth-250, 50)];
        [self addSubview:labelCurrentMonth];
        labelCurrentMonth.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:26];
        labelCurrentMonth.textColor = [UIColor colorWithRed:231.0f/255 green:231.0f/255 blue:231.0f/255 alpha:1.0];
        labelCurrentMonth.textAlignment = NSTextAlignmentCenter;
        labelCurrentMonth.backgroundColor = [UIColor colorWithHexString:@"035b85"];
